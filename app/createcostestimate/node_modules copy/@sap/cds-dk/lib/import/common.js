...
// Modify the output to replace the using statements with the dependency services
function _replaceUsingStatementsForDependent(output) {
    let using = usingStatements.join('\n');
    usingStatements = [];
    let startIndex = output.indexOf('using');
    let endIndex = output.indexOf(';', startIndex);
    let usingStatement = output.substring(startIndex, endIndex+1);
    output = output.replace(usingStatement, using);
    return output;
}
...
async function _getResult(output, as, dest, force, kind, isDependent) {
    ...
    if (as && as != "csn" && as != "json") {
        ...
        if (isDependent && kind === 'V4') {
            output = _replaceUsingStatementsForDependent(output);
        }
    }
...
async function postProcess(filePath, options, csn, cwd) {
    ...
    if (options.isDependency) {
        if (services.length > 1){
            usingStatements.push(`using { ${services.join(', ')}} from './${path.basename(csnFile)}';`);
        } else {
            usingStatements.push(`using { ${services} } from './${path.basename(csnFile)}';`);
        }
    }
