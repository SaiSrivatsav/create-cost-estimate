...
/**
 * Runs a before-script for the given task.
 * For now, this will only be used for cds watch,
 * but could be used for other tasks in the future,
 * therefore the generic name/ implementation.
 * @param {string} task - The task to run the before-script for.
 * @param {object} options - Options object containing the cwd.
 * @param {[string]} options.cwd - The current working directory.
 */
function runBeforeScript(task, {cwd}) {
  cwd ??= '.'
  try {
    const packageJson = getPackageJson(cwd)
    const beforeScript = `before:cds-${task}`
    if (packageJson.scripts?.[beforeScript]) {
      DEBUG(`running before-script '${beforeScript}' for cds ${task}`)
      const cmd = `npm run ${beforeScript} --prefix "${cwd}"`
      execSync(cmd)
    }
  } catch (e) {
    // ignore not finding package.json (happens during cds init)
    if (!(e.code === 'MODULE_NOT_FOUND' && e.message.includes('package.json'))) throw e
  }
}

/**
 * @param {string} project - The project to get the package.json from.
 * @returns {JSON} The package.json of the given project.
 */
function getPackageJson (project) {
  try {
    return require(path.join(project, 'package.json'))
  } catch (e) {
    if (e.code !== 'MODULE_NOT_FOUND') throw e
    return require(path.join(cds.root, project, 'package.json'))
  }
}